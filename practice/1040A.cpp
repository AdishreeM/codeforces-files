...
int main()
{
    ...
    for (i = 0; i < n / 2; i++)
    {
        ...
        {
            if (color[n - 1 - i] == 2)
                color[n - i - 1] = color[i], cost += (color[i] == 0 ? a : b);
            else if (color[n - 1 - i] != color[i])
                break;
            remaining -= 2;
        }
        else if (color[n - i - 1] != 2)
        {
            if (color[i] == 2)
                color[i] = color[n - 1 - i], cost += (color[n - 1 - i] == 0 ? a : b);
    ...
    {
        if (n % 2 == 1 && color[n / 2] != 2)
            remaining--;
        cost += min(a, b) * max(0, remaining);
        cout << cost << endl;
    }
}