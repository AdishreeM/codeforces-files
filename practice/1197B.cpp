...
int getMinIndx(VI nums)
{
    int n = nums.size();
    int maxm = nums[0], indx = 0;
    for (int i = 1; i < n; i++)
    {
        if(nums[i] > maxm)
        {
...
bool checkLeft(VI nums, int indx)
{
    int n = nums.size();
...
bool checkRight(VI nums, int indx)
{
    int n = nums.size();
...
int main()
{
    ...
    for (int i = 0; i < n; i++)
        cin >> nums[i];
    int indx = getMinIndx(nums);

    if(checkLeft(nums, indx) && checkRight(nums, indx))
        cout << "YES\n";
    else
        cout << "NO\n";
