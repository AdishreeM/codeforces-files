...
int main()
{
    ...
    {
        cin >> inp;
        string palindrome = getReverse(inp);
        if (words.find(palindrome) != words.end())
            words[palindrome] += 1;
        else
            words[inp] = 1;
    }
    ...
    {
        if (word.second > 1)
            repeated.PB(word.first);

        else if (isPalindrome(word.first))
        {
            if (word.second > max_size)
            {
                max_size = word.second;
                middle = word.first;
            }
        }
    }
    ...
    string ans;
    for (auto word : repeated)
        ans += word;
    if (max_size > 0)
        ans += middle;
    for (int i = repeated.size() - 1; i >= 0; i--)
        ans += getReverse(repeated[i]);
    cout << ans.size() << '\n'
         << ans << endl;
    return 0;
}