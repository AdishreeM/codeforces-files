...
vector<int> getOrder(int n, VI time, VI direction)
{
    if(n == 0)
        return {};
    int t = 0, dir = 1, next = 1, curr = 0;
    VI order(n, 0);
    while (next < n)
    {
        if(time[curr] == time[next])
        {
            if(direction[curr] == dir) {
                order[curr] = t++;
                curr = next;
                next++;
            } else {
