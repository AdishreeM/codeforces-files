...
long getMaxUnit(int num, vector<int> boxes, int unitSize, vector<int> unitsPerBox, long truckSize) {
    priority_queue<PII> products;
    for (int i = 0; i < num; i++)
        products.push({unitsPerBox[i], boxes[i]});
    long ans = 0;
    while(truckSize && products.size()) {
        auto prod = products.top();
        products.pop();
        long toAdd = min((long)prod.second, truckSize);
        ans += toAdd * prod.first;
        truckSize -= toAdd;
    }
    return ans;
}
