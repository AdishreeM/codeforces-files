...
int getMoves(VI arr, int pos)
{
    int count = 0, k = 0, n = arr.size();
    while (k < n && pos <= n * 2)
    {
        count += abs(arr[k] - pos);
        pos += 2, k++;
...
int main()
{
    ...
    int moves_black = getMoves(pos, 1);
    int moves_white = getMoves(pos, 2);
    cout << min(moves_black, moves_white) << endl;
    return 0;
}