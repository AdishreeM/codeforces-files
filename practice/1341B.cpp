...
int main()
{
    ...
    while (t--)
    {
        string inp;
        VS sudoku;
        VPII swaps = {{0, 0}, {1, 3}, {2, 6}, {3, 1}, {4, 4}, {5, 7}, {6, 2}, {7, 5}, {8, 8}};
        for (int i = 0; i < 9; i++)
        {
            cin >> inp;
            sudoku.PB(inp);
        }
        char temp = sudoku[0][0];
        for (int i = 0; i < 8; i++)
        {
            PII left = swaps[i], right = swaps[i + 1];
            sudoku[left.first][left.second] = sudoku[right.first][right.second];
        }
        sudoku[8][8] = temp;
        for (auto s : sudoku)
            cout << s << endl;
