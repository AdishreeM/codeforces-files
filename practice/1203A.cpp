...
bool isClockWise(VI nums)
{
    int n = nums.size(), start = findStartIndx(nums);
    for (int i = (start+1)%n; i != start; i=(i+1)%n)
    {
        int next_index = ((i == 0) ? n - 1 : i - 1);
        if(nums[i] != nums[next_index] + 1)
            return false;
    }
    return true;
}

bool isAntiClockWise(VI nums)
{
    int n = nums.size(), start = findStartIndx(nums);
    int i = ((start == 0) ? n - 1 : start - 1);
    while(i != start)
    {
...
int main()
{
    ...
    while (t--)
    {
        ...
        for (int i = 0; i < n; i++)
            cin >> nums[i];
        if(isClockWise(nums) || isAntiClockWise(nums))
            cout << "YES\n";
        else
            cout << "NO\n";        
    }
