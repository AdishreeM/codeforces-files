...
//
//
map<int, PII> getStats(VI arr) {
    int n = arr.size();
    map<int, PII> nums;
    for (int i = 0; i < n; i++)
    {
        if(nums.find(arr[i]) == nums.end())
            nums[arr[i]] = {(i==0? 0: 1), i};
        else {
            auto [interval, last] = nums[arr[i]];
            nums[arr[i]] = {interval + (i - last > 1? 1: 0), i};
        }
    }
    
    for(auto& num: nums) {
        if(num.second.second != n-1)
            num.second.first++;
    }
    return nums;
}

int getMinMoves(map<int, PII> nums) {
    int ans = INF;
    for(auto num: nums)
        ans = min(num.second.first, ans);
    return ans;
}
...
int main()
{
    ...
    while (t--)
    {
        ...
        for (int i = 0; i < n; i++)
            cin >> arr[i];
        auto nums = getStats(arr);
        int minm = getMinMoves(nums);
        cout << minm << endl;
    }
    return 0;
}